...
public class Dungeon2Piece extends TemplateStructurePiece {

    private static final ResourceLocation TEMPLATE =
            ResourceLocation.fromNamespaceAndPath(Constants.MOD_ID, "dungeon_t2");

    public Dungeon2Piece(StructureTemplateManager templateManager, BlockPos pos, Rotation rotation) {
        super(StructureRegistration.DUNGEON2_PIECE_TYPE, 0,
                templateManager, TEMPLATE, TEMPLATE.toString(),
                createPlacementSettings(rotation), pos);
    }

    public Dungeon2Piece(StructurePieceSerializationContext context, CompoundTag tag) {
        super(StructureRegistration.DUNGEON2_PIECE_TYPE, tag,
                context.structureTemplateManager(),
                (id) -> createPlacementSettings(Rotation.valueOf(tag.getString("Rot"))));
    }
